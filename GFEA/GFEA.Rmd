...
## Basic analysis
Take the Affymetrix Human Genome U95 Set annotation data (chip hgu95av2) for example. Assume we have a gene list $prbs$, the population list $univ$, the basic question: are these genes more often in the GO category (BP = Biological process; CC = Cellular component; MF = Molecular function) than expected by chance? Hypergeometric in package *GOstats* is used like:
...
if (length(row.names(summary(termCluster))) > 0){
  ...
  for (funclust in 1:length(row.names(summary(termCluster)))) {
    ...
    if (numNodes(goDag(davidGODag))>0){
      pdf(file=paste(prefix,'GeneOntologyFunctionalCluster',
                     funclust,'.pdf',sep=""))
