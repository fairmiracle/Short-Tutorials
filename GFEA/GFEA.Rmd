...
Recently I was trying to figure out what are the essential chararistics of the active modules with maxiaml summed gene expression level and correlation, which where identified by the package [AMOUNTAIN](http://bioconductor.org/packages/AMOUNTAIN) from weighted gene co-expression networks. Searching for known interactions inside a module might be a good idea. The package [STRINGdb](http://bioconductor.org/packages/STRINGdb) provide a set of API to access the resources of STRING by an effective way. Given a gene symbol list `mgenes` from homo sapiens, the following code works out ppi enrichment p-value and signigicantly enriched GO terms.
```{r,eval=FALSE}
library(STRINGdb)
...
genes_mapped <- string_db$map(diff_exp_genes, "gene",
