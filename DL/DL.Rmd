...
Deep learning (DL) has been a hot topic for a while. People tend to categorize it as a small (maybe not that small) branch in machine learning (ML), as a special form of neural networks (NN). But besides of the notable deep architectures that DL uses, what else has DL brought to ML? Or what are the essential differences between DL and traditional ML?

To answer this question, we may trace back to the origins of ML. Note that we can also trace back to the origins of neural networks as an exited story, from which we can see how neural networks models evolved into the current forms step by step. But the clue of ML makes it easier to understand why we need DL. In addition the conceptual aspect, we may also cover some details in the technical aspect on how DL is different from traditional ML. This post is also inspired by the [Deep Learning Specification](https://www.coursera.org/specializations/deep-learning) I took in the past several months, thank the financial aid provided by Coursera.

# The Conceptual part

A [traditional definition](http://www.cs.cmu.edu/~tom/mlbook.html) of ML requires a ML system (or algorithm) could learn from experience E (mostly data) on certain task T, and the ability of learning is measured by P. Learning from data is another summary of a ML algorithm. Different tasks in ML include supervised learning such as classification, unsupervised learning such as regression or reinforcement learning. The experience and performance measures vary on different tasks. 

The performance of a ML algorithm can be measured directly on experience (the training set), but we care more about its ability to predict given unseen data (test set), which is referred as generalization. For example, in supervised learning, training set comes with labeled data, but we hope the ML system could do as well on test set as possible. The so-called generalization ability is the real concern of a ML system. The traditional learning theory may tell you that simple ML models may underfit the data, which introduce high bias in testing stage. On the contrary, too complicated models may overfit the data, which introduce high variance. Neither high bias nor variance is expected, we need just right -- something between two cases. The following figure illustrates an example.
![Underfitting and overfitting, From Andrew Ng's Deep Learning Specification.](Fig1.png)


The ability of a ML system to fit the data is also referred as the model capacity, which is expected to capture the complexity of the data. If the model capacity is too high, overfitting occurs. ML has been struggling against overfitting for years, since people seem to worry more about applying too complicated algorithms on simple data, compared with applying too simple algorithms on complex data. Some useful techniques such as data augmentation, cross-validation and regularization have been developed to avoid overfitting.

But what if the data are always far more complex than expected? We are said in the big data era, when the data are explosively generated every day. How should ML evolve to handle with big data?
...
Instead of introducing more sophisticated hand-crafted features, people tend to use raw features as representation such as pixel values or raw speech signal. This so-called end-to-end learning has been one of the most exciting developments in DL. The following figure is a example of speech recognition.
![End-to-end learning in speech recognition. From Andrew Ng's ](Fig3.png)


## Evaluation
Most evaluation metrics for traditional ML such as precision, recall and error rate are still valid for DL, but the way to use them on building application can be different. In small data era, people used to split the whole available data into training and testing set by 70/30 percentages, or into training/development/test set by 60/20/20. But in big data era, we need to put most data training set to feed DL models, the split may become 98/1/1, even 1% of all data is enough for validation or testing. 
...
Like many people, I started my first complete course on machine learning with Prof. Andrew Ng in [CS229](http://cs229.stanford.edu/). Later he developed a webpage about [UFLDL Tutorial](http://deeplearning.stanford.edu/wiki/index.php/UFLDL_Tutorial) in around 2012 when deep learning began popular. From where I tried to learn something by implementing ideas, based on some starter code provided by Andrew. It turns out to be a really helpful way to figure out some details of an algorithm. Furthermore, by doing it one may get some confidence instead of the feeling to remember some scary terminologies. 
