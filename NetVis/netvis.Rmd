...
tao <- 0.8 # threshold to cut-off correlation value. Higher tao means sparser.
diag(w) <- 0
w[w<tao] <- 0
require(igraph)
...
It is a mass. Simply using `plot` leads to undesired results. We need to set the parametes with more careful consideration. If we add the requirement about edges width and vertices size,
```{r}
layout <- layout.reingold.tilford(g, circular=T)
plot(g,layout=layout,edge.width=(abs(w/max(w)))*5,vertex.size=10*abs((z/max(z))),
...
```{r}
setEPS()
postscript('toymodule.eps',fonts=c("serif", "Palatino"))
