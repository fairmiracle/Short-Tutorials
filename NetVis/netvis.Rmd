...
Basic visualization may not include all these features, but should contain any if possible. Let us do it step by step. We have stored three variables for illustration, a gene symbol list, a node score vecor and the Pearson correlation matrix between them. The correlation matrix is computed by substracting the subset from the whole gene expression profile (GSE35103) and use `cor`, and the node score is computed based on the expression level. Here is the basic visualization with $\mathtt{igraph}$

```{r}
load('toymodule.Rdata')
diag(w) <- 0
w[w<0.7] <- 0
require(igraph)
...
We can see that simply using `plot` leads to undesired results. We need to set the parametes with more careful consideration. If we add the requirement about edges width and vertices size,
```{r}
plot(g,edge.width=(abs(w/max(w)))*5,vertex.size=10*abs((z/max(z))),
...
```{r}
V(g)$color="gray"
V(g)[which(V(g)$name=='RORC')]$color="red"
V(g)[which(V(g)$name=='CCR6')]$color="red"
V(g)[grep('IL17',V(g)$name)]$color="red"
```

Or save the whole picture as EPS format by the following code. 
```{r}
setEPS()
postscript('conserved12h24h48h.eps',fonts=c("serif", "Palatino"))
plot(g,edge.width=(abs(w/max(w)))*5,vertex.size=10*abs((z/max(z))),
	vertex.label.dist = 0.5, vertex.label.cex = 0.75)
dev.off()
```

$\mathtt{igraph}$ also provide a simpler way to interacte with graph, one can move vertices or edges on the graph for better layout. Sometimes the labels are overlapped, `tkplot` allows us to modify it.
```{r,eval=FALSE}
tkplot(g,layout=layout_with_fr,canvas.width = 1000, canvas.height = 800,
    edge.width=(abs(w/max(w)))*5,vertex.size=15*abs((g/max(g))),
...
Sometimes we need to plot multilayer network as a globle picture of multple networks and their interactions. [$\mathtt{MuxViz}$](http://muxviz.net/) provides such a platform. 
