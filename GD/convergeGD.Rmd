...
Gradient descend is probably the most intuitive optimization algorithm. Just like the other methods, first we use it for granted, then we try to figure out why it works. The most important part of this reasoning is to prove it rigorously, which has been completed by mathematicians long time ago. But it is still necessary to write it down myself, to enhance my understanding.
...
We assue the optimal solution exists, i.e. $f({\bf x}^*)=minf({\bf x})$.
...
# A simple example
Image we want to minmize $f(x)=x^2$ in one-dimentional space, $\nabla f(x)=2x$, we set step size $t=0.01$, and maximal interation $T=1000$
...
$$
which can be derived from the Jensen's inequality, or a straitforward geometric interpretation.
...
This theorem is the main theoratical result of gradient descend, which guarantees the algorithm has convergence rate $O(1/k)$. In other words, the algorithm guarantees to approximate the $\epsilon$-accuracy, i.e. $f({\bf x}^{(k)})-f({\bf x}^*)\leq \epsilon$ in $O(1/\epsilon)$ iterations.
