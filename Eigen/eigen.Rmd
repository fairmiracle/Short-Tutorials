---
title: "Graph partitioning, modularity maximization and eigenvectors"
author: "Dong Li"
date: "June 26, 2017"
...
# Motivation
[Mark Newman](http://www-personal.umich.edu/~mejn/) is a leading research in the field of complex networks. Because of his fundamental contribution, networks have been a hot topic for years. Reading Mark Newman's papers is always enjoyable since they are easy to understand, but filled with insights. The fluent style and clear equations sometimes make me wrongly believe anyone could do that kind of research...Anyway, there are three papers (all sole author) talking about the same thing: the spectral algorithm for community detection.

 - [PNAS06] Modularity and community structure in networks. PNAS, 2006.
 - [PRE06] Finding community structure in networks using the eigenvectors of matrices. Physical review E, 2006.
 - [PRE13] Spectral methods for community detection and graph partitioning. Physical Review E, 2013.
...
Since $s_i$ are discrete values, maximizing Q is essentially a combinatorial problem. It is common to relax $\bf s$ to the real values. We are now solving an approximation which gives good results in practice. Also, note imposing constraints on $s_i$ is necessary otherwise we get an unbounded problem. The most commonly used L2-norm constraint $\sum_is_i=1$ actually limits any $s_i$ in the range $-\sqrt{n}\leq n \leq \sqrt{n}$. The problem is formally written as
$$\textrm{maximize}_{\bf s}\ Q={\bf s}^\top B {\bf s}\quad s.t.\quad \|{\bf s}\|_2=1$$
This problem is known as **spectral matching**, and the global optimum is given by the leading eigenvector of $B$. Newman used another logic to conclude the leading eigenvector of $B$ in PNAS06, which partly explain why eigenvector maximizes it. After getting the eigenvector $\bf u$, we divide the vertices into two groups according to the signs of elements in $\bf u$. 
...
$${\bf A}{\bf s} = {\bf D} \biggl(\lambda \vec{s} + {\vec{k}^T\vec{s}\over2m}
                 \vec{1} \biggr),$$

$${\bf A}{\bf s} = \lambda {\bf D} {\bf s}.
$$
We define a rescaled vector $\vec{u} = {\bf D}^{1/2}\vec{s}$, i.e., $\vec{s}={\bf D}^{-1/2}\vec{u}$. Then 
...
where $\kappa_1=vol(G_1)=\sum_{i\in G_1,j}w_{i,j}=\sum_{i\in G_1}k_i$ is the sum of degrees of vertices in group 1.
...
$$\sum_{ij} A_{ij} \biggl( s_i + \sqrt{\kappa_1\over\kappa_2} \biggr)
  \biggl( s_j - \sqrt{\kappa_2\over\kappa_1} \biggr)\qquad{} = -{(2m)^2\over\kappa_1\kappa_2} \sum_{ij} A_{ij}\,\delta_{g_i,1}\delta_{g_j,2}= -{(2m)^2\over\kappa_1\kappa_2} R,$$
...
[igraph](http://igraph.org/redirect.html) package implemented most community detection algorithms. It uses C as core language, R and Python interface languages, which considers both the efficiency and convenience. It even contains some graph dataset, including the famous ["Zachary" karate club network](http://networkkarate.tumblr.com). All these features make it a perfect tool in network science research.
...
Calling community detection algorithms in igraph is straightforward. Function `modularity` directly gives out the Q value for a partition. The implementation considers further division thus returns four communities, and the Q value is higher.
